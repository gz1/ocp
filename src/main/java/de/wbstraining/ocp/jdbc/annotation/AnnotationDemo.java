...
public class AnnotationDemo {
	public static void main(String[] args) throws Exception {
		...
		Class<?> cls = Class.forName(args[0]); // wbs.jdbc.annotation.Buch
		Column column;
		PrimaryKey primaryKey;
		// wir iterieren über a l l e
		// felder der klasse (auch private fields...)
		for (Field field : cls.getDeclaredFields()) {
			...
			if (column != null) {
				...
				// wir prüfen, ob das aktuelle feld die annotation @PrimaryKey
				// hat
				primaryKey = field.getAnnotation(PrimaryKey.class);
				if (primaryKey != null) {
					// das aktuelle feld hat die annotation @PrimaryKey
					// index() liefert den wert des attributs index:
					// @PrimaryKey(index = 1)
					System.out.println(field.getName() + " is column in primary key at index : " + primaryKey.index());
				}
			}
		}
	}
